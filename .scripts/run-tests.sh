...
check_output() {
	...
	if [ ! -f "$EXPECTED_OUTPUT" ] || [ ! -f "$RESULTED_OUTPUT" ]; then
		...
		diff -b "${RESULTED_OUTPUT%.*}.log" "${EXPECTED_OUTPUT%.*}.log" > "${RESULTED_OUTPUT%.*}.logdiff"
		...
		diff -b "$RESULTED_OUTPUT" "$EXPECTED_OUTPUT" > "${RESULTED_OUTPUT%.*}.diff"
		...
		if [[ "$LOG_PASSED" == "true" ]] && [[ "$GRL_PASSED" == "true" ]]; then
...
if [ -z "$TEST_NUM" ]; then
	for file in "$INPUT_PATH/"*.txt; do
		...
		$BIN "$EDD" "$file" "$TEST_OUTPUT_PATH/$OUTPUT_FILE" > "$TEST_OUTPUT_PATH/${OUTPUT_FILE%.*}.log"

		check_output "$TEST_OUTPUT_PATH/$OUTPUT_FILE" "$OUTPUT_PATH/$OUTPUT_FILE"
	done
else
	INPUT_FILE=$(ls "$INPUT_PATH/$TEST_NUM"*.txt)
	OUTPUT_FILE=$(basename "$INPUT_FILE")

	$BIN "$EDD" "$INPUT_FILE" "$TEST_OUTPUT_PATH/$OUTPUT_FILE" | tee "$TEST_OUTPUT_PATH/${OUTPUT_FILE%.*}.log"

	echo -e "\n=== ($INPUT_FILE): TEST START ===\n"
	cat "$TEST_OUTPUT_PATH/$OUTPUT_FILE"
	echo -e "\n=== ($INPUT_FILE): TEST END   ===\n"
